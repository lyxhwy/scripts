...
return result}function processProxies(proxies){
const groupedProxies=proxies.reduce((groups,item)=>{
const existingGroup=groups.find(group=>group.name===item.name);
if(existingGroup){
existingGroup.count++;
existingGroup.items.push({...item,name:${item.name} ${existingGroup.count.toString().padStart(2,"0")}});
}else{
...
return proxies}function getFlagEmoji(cc){
...
return groups},{});for(const name in groups){
